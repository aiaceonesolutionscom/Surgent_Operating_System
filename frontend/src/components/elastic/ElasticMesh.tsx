...
void main() {
  vec3 base;
  if (uHasImage > 0.5) {
    float sx = max(uImageAspect / uAspect, 1.0);
    float sy = max(uAspect / uImageAspect, 1.0);
    vec2 uv = vec2(0.5 + (vUv.x - 0.5) / sx, vUv.y / sy);
    base = texture2D(tMap, uv).rgb;
  } else {
...
const ElasticMesh = ({
  ...
  useEffect(() => {
    ...
    function toPlane(clientX: number, clientY: number) {
      ...
      const clipX = mx * 2 - 1;
      const clipY = 1 - my * 2;
      const t = ((propsRef.current.tilt as number) || 0) * (Math.PI / 180);
      const ct = Math.cos(t);
      const st = Math.sin(t);
    ...
    function substep() {
      ...
      const retain = 1 - (p.damping as number);
      const coupling = 0.06 + (p.wobble as number) * 0.032;
      const active = pointer.active && (p.enabled as boolean) && !reduceMotion;
      const r = Math.max(0.08, p.grabRadius as number) * 1.4;
      const invR = 1 / r;
      const force = (p.pull as number) * 0.009;
    ...
    function frame(now: number) {
      ...
      program.uniforms.uColor1.value = hexToRgb(p.color1 as string);
      program.uniforms.uColor2.value = hexToRgb(p.color2 as string);
      program.uniforms.uHighlight.value = hexToRgb(p.highlight as string);
      ...
      program.uniforms.uGridColor.value = hexToRgb(p.gridColor as string);
