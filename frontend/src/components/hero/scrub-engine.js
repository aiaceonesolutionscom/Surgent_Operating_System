...
function mountLetsScroll(container, config) {
  ...
  // fire after teardown (belt-and-suspenders; removeEventListener below is what
  // actually stops them in normal use).
  let disposed = false;
  ...
  // Tears down everything this call registered: the rAF loop, and every
  // `window` listener. Without this, a framework that mounts/unmounts this
  // container (e.g. client-side routing away and back) leaks a full new set
  // of listeners and an orphaned rAF loop scrubbing DOM that's already gone
